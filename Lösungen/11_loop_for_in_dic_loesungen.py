...
print("-" * 30)
# Gegeben ist das folgende Dictionary student_scores: {'Alice': [80, 90, 70], 'Bob': [85, 88, 75], 'Charlie': [95, 82, 88]}. Schreibe eine Schleife, die den Durchschnitt der Noten für jeden Schüler berechnet und ausgibt.
student_scores = {'Alice': [80, 90, 70], 'Bob': [85, 88, 75], 'Charlie': [95, 82, 88]}
...
best_student = "Alice"
best_punkte = student_scores["Alice"][0]

for student, noten in student_scores.items():
    for punkt in noten:

        if punkt > best_punkte:
            best_punkte = punkt
            best_student = student
print(f"beste Note {best_student} mit {best_punkte}")
print("-" * 30)
...
for keys, values in student_scores.items():
    ...
    for note in values:
        if note > max_wert:
            ...
            min_wert = note
    print(f'{keys} min:{min_wert} max:{max_wert}')
print("-" * 30)
